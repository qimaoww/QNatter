...
printf 'QNATTER_INSTANCE=dotXname\0' > "$proc_dir/100/environ"
printf 'QNATTER_STATUS_FILE=%s/wan_ct.json\0' "$run_dir" > "$proc_dir/101/environ"
...
if ! output="$(
	...
	QNATTER_PROC_DIR="$proc_dir" \
	"$ROOT/luci-app-qnatter/root/usr/libexec/qnatter-status"
)" 2>"$stderr"; then
	cat "$stderr" >&2
	fail "qnatter-status returned an error"
fi

if [ -s "$stderr" ]; then
	cat "$stderr" >&2
	fail "qnatter-status must not write proc scan races to stderr"
fi
