...
run_hotplug() {
	...
	NATTER_INIT_BIN="$init_bin" \
		sh "$ROOT/natter/files/natter.hotplug"
}
...
if [ -s "$init_log" ]; then
	fail "disabled matching instance must not reload Natter"
fi

run_hotplug ifdown wan pppoe-wan
if [ -s "$init_log" ]; then
	fail "ifdown must not reload Natter"
fi

printf 'natter hotplug checks passed\n'
