...
func TestRunOnceReportsInitialTCPPortChecks(t *testing.T) {
	...
	if result.Ports.QNatterLAN != PortOpen {
		t.Fatalf("qnatter LAN check = %v, want open", result.Ports.QNatterLAN)
	}
...
func TestRunOnceNoneForwardTargetsQNatterAddressForUDP(t *testing.T) {
	stunClient := &fakeSTUN{
		mappings: []stun.Mapping{
			{
	...
	if notifyMapping.Inner != netip.MustParseAddrPort("10.0.0.2:50000") {
		t.Fatalf("notify inner = %s, want qnatter address", notifyMapping.Inner)
	}
}
