...
func TestRunVersion(t *testing.T) {
	...
	if got := stdout.String(); !strings.Contains(got, "QNatter Go") {
		t.Fatalf("version output = %q, want QNatter Go", got)
	}
}
...
func TestRunCheckParsesExistingOpenWrtArguments(t *testing.T) {
	...
	code := runWithContext(context.Background(), []string{
		...
		got = cfg
		fmt.Fprintln(out, "> QNatterCheck Go")
		return nil
	})
	...
	if !strings.Contains(stdout.String(), "QNatterCheck Go") {
...
func TestRunRejectsUnknownArgumentsWithoutWritingGlobalStderr(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), "qnatter:") {
		t.Fatalf("stderr = %q, want qnatter error", stderr.String())
	}
}
...
func TestRunLogsUseTimestampedLevels(t *testing.T) {
	...
	infoLine := regexp.MustCompile(`^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2} \[I\] QNatter v`)
	errorLine := regexp.MustCompile(`^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2} \[E\] qnatter: mapping failed$`)
...
func TestLogNotifyScriptPrintsPathWhenConfigured(t *testing.T) {
	var stderr bytes.Buffer
	logNotifyScript(&stderr, "/var/run/qnatter/default.notify")

	if !strings.Contains(stderr.String(), "[I] Calling script: /var/run/qnatter/default.notify") {
...
func TestLogMappingPrintsRouteInformation(t *testing.T) {
	...
	for _, want := range []string{
		"[I] tcp://10.10.10.10:51413 <--socket--> tcp://10.10.10.2:40000 <--QNatter--> tcp://203.0.113.10:62000",
	} {
		if !strings.Contains(stderr.String(), want) {
			t.Fatalf("stderr = %q, missing %q", stderr.String(), want)
		}
...
func TestLogMappingOmitsForwardSegmentForNoneMethod(t *testing.T) {
	...
	want := "[I] udp://10.0.0.2:50000 <--QNatter--> udp://198.51.100.8:62001"
	if !strings.Contains(stderr.String(), want) {
		t.Fatalf("stderr = %q, missing %q", stderr.String(), want)
	}
...
func TestRunWithContextTreatsExitWhenChangedAsCleanExit(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), "[I] QNatter is exiting because mapped address has changed") {
...
func TestRunWithContextTreatsLocalAddressChangeAsCleanExitWhenRequested(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), "[I] QNatter is exiting because local IP address has changed") {
