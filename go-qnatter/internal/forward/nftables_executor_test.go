...
func TestNftablesForwarderStartAndStopDNAT(t *testing.T) {
	runner := &fakeNftRunner{
		outputs: map[string]string{
			NftablesDNATRule(nftTestOptions()): "insert rule ip qnatter qnatter_dnat # handle 42\n",
		},
	}
	...
	wantCalls := []string{
		"list table ip qnatter",
		NftablesDNATRule(nftTestOptions()),
		"delete rule ip qnatter qnatter_dnat handle 42",
	}
...
func TestNftablesForwarderMarksRepliesWithExplicitRoutePolicy(t *testing.T) {
	...
	runner := &fakeNftRunner{
		outputs: map[string]string{
			NftablesDNATRule(options):                   "insert rule ip qnatter qnatter_dnat # handle 42\n",
			NftablesConnMarkRule(options, policy.Mark):  "insert rule ip qnatter qnatter_mark # handle 87\n",
			NftablesRouteMarkRule(options, policy.Mark): "insert rule ip qnatter qnatter_mark # handle 88\n",
		},
	}
	ip := &fakeIPRunner{}
	f := testNftablesForwarder(runner)
	...
	wantCalls := []string{
		...
		"list chain ip qnatter qnatter_mark",
		NftablesConnMarkRule(options, policy.Mark),
		NftablesRouteMarkRule(options, policy.Mark),
		"delete rule ip qnatter qnatter_dnat handle 42",
		"delete rule ip qnatter qnatter_mark handle 87",
		"delete rule ip qnatter qnatter_mark handle 88",
	}
	assertNftCalls(t, runner.calls, wantCalls)
...
func TestQNatterRoutePolicySeparatesInstanceSlots(t *testing.T) {
	mc := qnatterRoutePolicy(2)
	qb := qnatterRoutePolicy(0)
...
func TestNftablesForwarderStartAndStopSNAT(t *testing.T) {
	...
	runner := &fakeNftRunner{
		outputs: map[string]string{
			NftablesDNATRule(options):     "insert rule ip qnatter qnatter_dnat # handle 42\n",
			NftablesSNATRule(snatOptions): "insert rule ip qnatter qnatter_snat # handle 77\n",
		},
	}
	f := testNftablesForwarder(runner)
	f.SNAT = true
	...
	wantCalls := []string{
		...
		"delete rule ip qnatter qnatter_dnat handle 42",
		"delete rule ip qnatter qnatter_snat handle 77",
	}
	assertNftCalls(t, runner.calls, wantCalls)
}
...
func TestNftablesForwarderCreatesTableWhenMissing(t *testing.T) {
	...
	runner := &fakeNftRunner{
		errors: map[string]error{
			"list table ip qnatter": errors.New("table missing"),
		},
		outputs: map[string]string{
			NftablesDNATRule(options): "insert rule ip qnatter qnatter_dnat # handle 42\n",
		},
	...
	if len(runner.calls) < 2 || !strings.Contains(runner.calls[1], "table ip qnatter") {
...
func TestNftablesForwarderRollsBackDNATWhenSNATFails(t *testing.T) {
	...
	wantDelete := "delete rule ip qnatter qnatter_dnat handle 42"
